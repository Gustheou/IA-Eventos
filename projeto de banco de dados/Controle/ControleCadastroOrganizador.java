  ...
  void cadastrarOrganizadorButton(ActionEvent event) {
    try{
      ...
      if (nome.equals("") || String.valueOf(cpf).equals("") || (String.valueOf(cpf).length() != 11)){
        ...
        try {
          //Inserir no bd
          String query = "INSERT INTO iaeventos.organizador (nome, cpf, nome_evento) VALUES ('"+nome+"', '"+cpf+"', '"+nomeDoEvento+"')";
  ...
  void eventOnActionMenuButton(MouseEvent event) {
    try {
      ...
      try {
        ...
        System.out.println("Exception no cadastro organizador: " + e.getMessage());
      }
      operacao.close();
    } catch (SQLException e) {
      System.out.println("SQLException: " + e.getMessage());
